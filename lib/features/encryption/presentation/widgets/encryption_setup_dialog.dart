...
/// recovery code. Pops with `true` when setup completes.
...
class _EncryptionSetupDialogState extends State<EncryptionSetupDialog> {
  ...
  Future<void> _submitPassphrase() async {
    ...
    final result = await cubit.setup(passphrase);
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          ...
          style: TextStyle(
              fontSize: 14.0, color: mainTextColor, fontWeight: FontWeight.w600),
        ),
        const SizedBox(height: 12),
        Text(
          "Encrypted notes are excluded from search.",
  ...
  Widget _buildPassphraseForm(Color mainTextColor) {
    return Column(
      ...
      children: [
        ...
        Text(
          "You'll enter this to unlock encrypted notes. Use something long and memorable - it cannot be recovered.",
          style: TextStyle(fontSize: 13.0, color: mainTextColor),
        ),
        const SizedBox(height: 14),
        TextField(
